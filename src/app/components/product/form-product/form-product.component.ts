...
export class FormProductComponent {
  ...
  submit(redirect: boolean = true) {
    ...
    if (this.form.controls['type'].value === 'classified') {
      const variations = (this.form.get('variations') as FormArray).value as any[];
      if (variations && variations.length > 0) {
        const salePrices = variations.map((v: any) => {
        ...
        const minIndex = salePrices.indexOf(minSalePrice);
        formValue['sale_price'] = minSalePrice.toFixed(2);
        formValue['price'] = parseFloat(variations[minIndex]?.price) || minSalePrice;
      }
    } else {
